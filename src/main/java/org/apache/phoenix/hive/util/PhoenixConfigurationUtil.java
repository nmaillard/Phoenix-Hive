...
public final class PhoenixConfigurationUtil {
    ...
    private static final Log LOG = LogFactory.getLog(PhoenixConfigurationUtil.class);
    ...
    public static void
            setSelectColumnNames(final Configuration configuration, final String columns) {
        Preconditions.checkNotNull(configuration);
    ...
    public static void
            setOutputTableName(final Configuration configuration, final String tableName) {
        Preconditions.checkNotNull(configuration);
        Preconditions.checkNotNull(tableName);
    ...
    public static void
            setUpsertColumnNames(final Configuration configuration, final String columns) {
        Preconditions.checkNotNull(configuration);
    ...
    public static List<ColumnInfo> getUpsertColumnMetadataList(final Configuration configuration)
        ...
        if (isNotEmpty(upsertColumns)) {
            final String columnNamesDelimiter =
                    configuration.get(COLUMN_NAMES_DELIMITER, DEFAULT_COLUMN_NAMES_DELIMITER);
            upsertColumnList =
                    Lists.newArrayList(Splitter.on(columnNamesDelimiter).omitEmptyStrings()
                            .trimResults().split(upsertColumns));
            LOG.info(String
                    .format(
                        "UseUpsertColumns=%s, upsertColumns=%s, upsertColumnSet.size()=%s, parsedColumns=%s ",
                        !upsertColumnList.isEmpty(), upsertColumns, upsertColumnList.size(), Joiner
                                .on(DEFAULT_COLUMN_NAMES_DELIMITER).join(upsertColumnList)));
        }
    ...
    public static String getUpsertStatement(final Configuration configuration) throws SQLException {
        ...
        while (it.hasNext()) {
            ...
            LOG.info("Phoenix entry key: " + e.getKey() + " val: " + e.getValue());
        }
        final String tableName = getOutputTableName(configuration);
        Preconditions.checkNotNull(tableName);
        ...
        if (useUpsertColumns) {
            ...
            // Generating UPSERT statement without column name information.
            upsertStmt =
                    QueryUtil.constructGenericUpsertStatement(tableName, columnMetadataList.size());
    ...
    private static List<String> getSelectColumnList(final Configuration configuration) {
        ...
        if (isNotEmpty(selectColumns)) {
            ...
            selectColumnList =
                    Lists.newArrayList(Splitter.on(columnNamesDelimiter).omitEmptyStrings()
                            .trimResults().split(selectColumns));
            LOG.info(String
                    .format(
                        "UseSelectColumns=%s, selectColumns=%s, selectColumnSet.size()=%s, parsedColumns=%s ",
                        !selectColumnList.isEmpty(), selectColumns, selectColumnList.size(), Joiner
                                .on(DEFAULT_COLUMN_NAMES_DELIMITER).join(selectColumnList)));
        }
